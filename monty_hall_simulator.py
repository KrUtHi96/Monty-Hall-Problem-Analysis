...
def simulate(num_doors, switch, steps, winning_door, choice):
	...
	#assert (0 <= winning_door <= num_doors-1), 'Choice for winning door invalid'
...
def main():
	...
	for i in range(args.trials):
		...
		won = simulate(args.doors, switch=False, steps=args.steps, winning_door=(args.win_ch-1), choice=(args.con_ch-1))
		if won:
			winning_non_switchers += 1
		
		# Next, try one where the contestant switches.
		won = simulate(args.doors, switch=True, steps=args.steps, winning_door=(args.win_ch-1), choice=(args.con_ch-1))
